...
(function (global, factory) {
    ...
    // explicitness and function inlining（显示和函数内联）
    ...
    function toString(val) {
        return val == null ?
            '' :
            typeof val === 'object' ?
            //JSON.stringify，第一个参数为传入数据，第二个参数为过滤函数或者数组，第三个参数为转换字符串之后字符串之间的间隔
     ...
     * 用连字符分隔的字符串。
     * 调用一个camelize 存一个键进来 调用两次 如果键一样就返回hit 
     * 横线-的转换成驼峰写法
     ...
     * 检测a和b数据是否大致相等，如果是object，对象的key长度是否相等，数组长度是否相等
     */
     ...
     * 主要参考Object.defineProperty方法（重要）
     */
     ...
     * 解析简单路径
     */
    var bailRE = /[^\w.$]/; //匹配开头不是字母、数字、下划线和$符号的数据
    ...
    // Firefox 在Object.prototype上具有“监视”功能(应该是不允许操作Object.prototype(懒得去验证了))
     ...
     * observer：用于劫持并监听所有属性  dep：用于添加订阅者，订阅发布者变化 observer监听到变化并通知dep
     * 主题对象Dep构造函数  主要用于添加发布事件后，用户更新数据的 响应式原理之一函数
     */
    var Dep = function Dep() {
        //uid  初始化为0
        this.id = uid++;
        /* subs:用来存放订阅者Watcher对象的数组 */
        this.subs = [];
    };
    ...
    */
    ...
    //优化浅拷贝
    //用于静态节点和插槽节点，因为它们可以被复用，避免操作DOM节点时因为相同引用造成和错误
    ...
    function cloneVNode(vnode, deep) {
        ...
        cloned.isCloned = true; /*标记是否为克隆节点*/
        if (deep) { //如果deep存在
     ...
     * 在某些情况下，我们可能希望禁用组件更新计算中的监听
     */
    var shouldObserve = true; //标志是否禁止监听还是添加到订阅者模式
     ...
     * 添加到每个观察对象的Observer类。 
     * 添加后，订阅者会将目标对象的属性键加入订阅集合并监听变化的getter / setter。
     * 实例化dep对象,获取dep对象为value添加__ob__ 属性
     * 重要知识点，涉及双向绑定原理
     */
    var Observer = function Observer(value) {
        ...
        this.vmCount = 0;
        //设置监听 value 必须是对象
        def(value, '__ob__', this); //将value转换为对象
        //以下方法可以解释为何Vue中监测不到数组变化
        //数组的下标变化其实是可以监测的，但是：性能代价和获得的用户体验收益不成正比，因此跳过对数组的每个元素进行监听
        if (Array.isArray(value)) { //判断是不是数组
            var augment = hasProto //__proto__ 存在么 高级浏览器都会有这个
                ?
                protoAugment :
                copyAugment;
            augment(value, arrayMethods, arrayKeys); //数组增强函数？
     ...
     * 尝试为某个值创建一个订阅者实例，如果成功观察到该订阅者，则返回新订阅者，如果该值已经具有一个订阅者，则返回现有订阅者。
     * 判断value 是否有__ob__ 实例化 dep对象,获取dep对象为value添加__ob__ 属性  返回 new Observer 实例化的对象
     */
    ...
    function defineReactive(obj, //对象
        ...
        Object.defineProperty(obj, key, {
            ...
            get: function reactiveGetter() {
                ...
                if (Dep.target) { //Dep.target 静态标志 标志了Dep添加了Watcher实例化的对象
                    //添加一个dep
                    dep.depend();
     ...
     * 在对象上设置属性。如果属性不存在，则添加新属性，并触发更新方法
     */
    ...
    function set(target, key, val) {
        ...
        //判断key是否在target 上，并且不是在Object.prototype 原型上
     ...
     * 数组需要设置特定的依赖，因为数组不能元素不能想对象元素一样设置getters
     */
    ...
    /**
     * Option overwriting strategies are functions that 
     * handle how to merge a parent option value and a child option value into the final value.
     ...
     * 监视程序解析表达式，收集依赖项并在表达式值更改时触发回调。这用于$ watch（）api和指令。
